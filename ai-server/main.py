...
rag_prompt_template = ChatPromptTemplate.from_messages(
    [
        (
            ...
            1. 이전 대화가 있다면 대화의 맥락을 이해하는 데 활용해라.

            2. 검색된 참고자료가 사용자의 질문과 관련이 있다면
               참고자료의 내용을 우선적으로 활용해서 답변해라.

            3. 검색된 참고자료가 질문과 관련이 없거나
               필요한 정보가 참고자료에 없다면
               참고자료에 억지로 맞추지 말고
               너의 일반적인 지식을 이용해서 답변해라.

            4. 참고자료와 일반 지식을 구분해서 판단하고,
               관련 없는 참고자료 때문에 답변을 거부하지 마라.

            5. 현재 날씨, 실시간 뉴스, 현재 주가, 최신 환율처럼
               실시간 정보가 필요한 질문은
               현재 제공된 정보만으로 정확히 확인할 수 없다면
               최신 정보가 필요하다고 솔직하게 알려라.
               실시간 정보를 추측해서 만들어내지 마라.

            6. 확실하지 않은 정보를 사실처럼 만들어내지 마라.

            7. 사용자의 질문에 직접적으로 답하고,
               불필요하게 참고자료의 존재 여부를 언급하지 마라.
            """
        ),
        (
            "human",
